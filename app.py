...
# 2. 디자인 스타일 적용 (원본 레이아웃 유지 + 하이라이트 + 링크 스타일)
st.markdown("""
    <style>
    ...
    /* 원본 스타일 보존: 모든 stCode 박스의 줄바꿈 강제 설정 */
    ...
    /* 정답 (☓) 지문 주황색 하이라이트 강제 적용 (복사 버튼 유지형) */
        ...
        for block in blocks:
            ...
            if parsed_data:
                ...
                with st.container(border=True):
                    ...
                    with col2:
                        st.markdown("<div class='section-title'>⚖️ 판례 / 조문 번호 (클릭 시 검색)</div>", unsafe_allow_html=True)
                        ...
                        if p_num != "근거 확인 필요":
                            ...
                            # 클릭 가능한 영역 생성 (st.code의 시각적 일관성 유지)
                            st.markdown(f"""
                                <a href="{search_url}" target="_blank" class="link-box">
                                    <div style="background-color: #f0f1ff; border: 1px solid #6366f1; border-radius: 16px; padding: 22px; color: #6366f1; font-weight: 700; text-align: center;">
                                        🔗 {p_num}
                                    </div>
                                </a>
                            """, unsafe_allow_html=True)
